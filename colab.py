...
def measure_resource_utilization(python_script='script.py', sampling_interval=1, output_file="utilization.csv", device = 'cuda'):
    ...
    with open(output_file, mode='w', newline='') as cpu_file, open(process_output_file, mode='w', newline='') as process_file:
        ...
        cpu_writer.writerow(["Date", "Time", "Process", "cpu_util", "memory_util", "user", "system", "nice", "idle", "iowait", "IRQ", "softIRQ",
        ...
        try:
            ...
            while process.poll() is None:  # Poll will return None while the script is running
                ...
                if "MiB Mem" in cpu_line:
                  ...
                  cpu_writer.writerow([timestamp.strftime('%Y-%m-%d'), timestamp.strftime('%H:%M:%S.%f'), "all", cpu_usage, memory_util,
